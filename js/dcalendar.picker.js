	...
	//	daysofweek = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'],
		...
		DCalendar = function(elem, options) {
			...
			//	this.date = isNaN(parseInt(dateObj.m)) ? new Date(dateObj.m + " " + dateObj.d + ", " + dateObj.y) : new Date(dateObj.y, dateObj.m - 1, dateObj.d);
				this.date = isNaN(parseInt(dateObj.m)) ? new Date(dateObj.y + " " + dateObj.m + ", " + dateObj.d) : new Date(dateObj.y, dateObj.m - 1, dateObj.d);
			}

			this.viewMode = 'days';
			this.options = options;
		//	this.selected = (this.date.getMonth() + 1) + "/" + this.date.getDate() + "/" + this.date.getFullYear();
			this.selected = this.date.getFullYear() + "/" + (this.date.getMonth() + 1) + "/" + this.date.getDate();
	...
	DCalendar.prototype = {
		...
		setDate : function() {
			...
			//	value = isNaN(parseInt(dateObj.m)) ? new Date(dateObj.m + " " + dateObj.d + ", " + dateObj.y) : new Date(dateObj.y, dateObj.m - 1, dateObj.d);
				value = isNaN(parseInt(dateObj.m)) ? new Date(dateObj.y + " " + dateObj.m + ", " + dateObj.d) : new Date(dateObj.y, dateObj.m - 1, dateObj.d);

		//	that.selected = (value.getMonth() + 1) + "/" + value.getDate() + "/" + value.getFullYear();
			that.selected = value.getFullYear() + "/" + (value.getMonth() + 1) + "/" + value.getMonth();
			that.selectDate();
			that.date = value;
			that.create(that.viewMode);
		},
		...
		create : function(mode){
			...
			//	var sysDate = "Today: " + daysofweek[that.today.getDay()] + ", " + months[that.today.getMonth()] + " " + that.today.getDate() + ", " + that.today.getFullYear();
				var sysDate = "今天: "+that.today.getFullYear()  + ", "+ months[that.today.getMonth()] + " " + that.today.getDate() + ", " + daysofweek[that.today.getDay()];
